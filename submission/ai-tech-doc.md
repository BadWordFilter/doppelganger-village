...
| 도구 | 용도 |
|---|---|
| **Claude Code (Fable 5)** | 전체 개발 주도 — 코드, 씬 구축, 검증, 디버깅, 문서 |
| **MCP for Unity** (CoplayDev, MIT) | Claude ↔ Unity 에디터 브리지 (스크립트/씬/컴포넌트/플레이 모드/콘솔/빌드) |
| Python (pdfplumber, pypdf) | 기획서 PDF에서 대화 테이블 추출 |
| **Kiwi 형태소 분석기** (kiwipiepy) | PDF 추출 시 소실된 한국어 띄어쓰기를 줄바꿈 접합부에만 국소 복원 |

## 3. 핵심 활용 사례

### 3-1. 기획서 대화 테이블 → 게임 데이터 (원문 무손실 이식)
게임의 최대 자산인 대화 테이블 67개(강아지/고양이/토끼 × 일상/핵심 질문·정상/이상 답변)를 기획서 PDF에서 `dialogue.json`으로 이식했습니다.
- pdfplumber 표 인식 → 페이지 경계에 잘린 2개 행은 원문 텍스트에서 수동 복원
- PDF 추출 시 줄바꿈 지점에서 사라지는 띄어쓰기를 **Kiwi 형태소 분석을 접합부에만 적용**해 원문 훼손 없이 복원
- 검증 2중화: ① 전 항목을 공백 제거 후 PDF 원문과 부분 문자열 자동 대조 ② 67개 전량 육안 검수 (수동 보정 2건)
...
그 외 그래픽은 전부 Unity 프리미티브 + 코드 생성 머티리얼 (외부 모델·텍스처 없음). 게임 내 대화 텍스트는 자체 기획서 원문.
